...
import { ArrowLeft, ArrowRight, Check, User, Phone, MapPin, Loader2 } from "lucide-react";
...
export default function Signup() {
  ...
  const submit = async () => {
    setLoading(true);
    try {
      const client = await addClient({ name, phone, city });
      if (client) {
  ...
  const canProceed = [name.trim().length > 1, phone.length > 5, city.length > 0];
      ...
      <div className="min-h-screen flex items-center justify-center pt-16 px-4">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md">
          <div className="glass rounded-2xl p-8 glow-box-cyan">
            <h2 className="text-2xl font-bold text-foreground text-center mb-6">{t("signup.title")}</h2>
            {step < 3 && (
              ...
              <motion.div key={step} custom={dir} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }}>
                {step === 0 && (
                  <div className="space-y-4">
                    ...
                    <Input value={name} onChange={(e) => setName(e.target.value)} placeholder={t("signup.namePlaceholder")} className="bg-secondary border-border text-foreground" />
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={next} disabled={!canProceed[0]} className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold flex items-center justify-center gap-2 disabled:opacity-40">
                ...
                {step === 2 && (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {cities.map((c) => (
                        <motion.button key={c} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => setCity(c)} className={`py-3 rounded-xl text-sm font-medium transition-colors ${city === c ? "bg-primary text-primary-foreground" : "bg-secondary text-foreground hover:bg-muted"}`}>
                          {t(`city.${c}` as any)}
                        </motion.button>
                      ))}
                    </div>
                    <div className="flex gap-3">
                      ...
                      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={submit} disabled={!canProceed[2] || loading} className="flex-1 py-3 rounded-xl bg-primary text-primary-foreground font-semibold flex items-center justify-center gap-2 disabled:opacity-40">
                ...
                {step === 3 && result && (
                  <div className="text-center space-y-6">
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-16 h-16 rounded-full bg-primary/20 flex items-center justify-center mx-auto">
                    ...
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => navigate("/dashboard")} className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold">
                      {t("nav.dashboard")} →
                    </motion.button>
                  </div>
                )}
